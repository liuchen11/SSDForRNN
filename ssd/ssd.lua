...
matrixNorm=require 'util.matrixNorm'
vectorNorm=require 'util.vectorNorm'
sharp=require 'util.sharp'

--stochastic spectral descent
function ssd(model,states,ground_truth,num,learning_rate)
	--model: RNN model
	...
	for iter=1,num do
		...
		--calculate L_dV
		local L_dV=math.pow(vectorNorm(present_states,2),2)/2
		model.h2o.gradWeight=model.h2o.gradWeight+dV_sharp/L_dV
		...
		local input_2n=vectorNorm.norm(states[iter])		--|x_t|_2
		local W_sinf=matrixNorm.norm(W,1/0)					--|W|_S^\inf
		...
		local PWV_s1=matrixNorm.norm(PWV:squeeze(),1)
		...
		for i=1,hidden_size do
			PW_s1_vec[i]=matrixNorm.norm(PW[{{},{},{i,i}}]:squeeze(),1)
		end
		...
		local past_dSdW_bar_s1=matrixNorm.norm(past_dSdW_bar,1)
		...
		model.h2h.gradWeight=model.h2h.gradWeight+dW_sharp/L_dW

		local QW=torch.bmm(past_dSdU,batchWT)
		local QWV=torch.bmm(QW,batchVp)
		local QWV_s1=matrixNorm.norm(QWV:squeeze(),1)
		...
		for i=1,hidden_size do
			QW_s1_vec[i]=matrixNorm.norm(QW[{{},{},{i,i}}]:squeeze(),1)
		end
		...
		local past_dSdU_bar_s1=matrixNorm.norm(past_dSdU_bar,1)
		...
		model.i2h.gradWeight=model.i2h.gradWeight+dU_sharp/L_dU
	end
